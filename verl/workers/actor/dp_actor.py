...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch: Dict[str, torch.Tensor], temperature: float) -> torch.Tensor:
        ...
        if self.config.padding_free:
            ...
            ################################################################
            #
            # last layer attn weight
            #
            ####################################################################
            # if not hasattr(output, "hidden_states") or output.hidden_states is None:
            #     raise RuntimeError("output 中缺少 hidden_states，请设置 output_hidden_states=True")

            # second_last_hidden_state = output.hidden_states[-2]  # shape: (1, seq_len, hidden_dim)


            # last_layer = self.actor_module.model.layers[-1]       
            # self_attn = last_layer.self_attn                     

            # attn_mask = None                                     
            # pos_ids = position_ids_rmpad                         
            
            # attn_output, attn_weights, past_key_value = self_attn(
            #     hidden_states=second_last_hidden_state,
            ...
            # last_attention_weight = attn_weights
            ####################################################################################################
